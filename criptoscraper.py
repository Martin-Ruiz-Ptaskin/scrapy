    ...
    def func(threads,url):
       try: 
         ...
         for u in url:     
           ...
           autos = driver.find_elements(By.XPATH,'//div[@class="account-hash-content w-100"]')
           for auto in autos:
                    # Por cada anuncio hallo el preico
            precio = auto.find_elements(By.XPATH,'.//span[@class="wb-ba"]')
            precio=precio[0].name
            print(precio)
            wallet = auto.find_elements(By.XPATH,'.//div[@class="account-hash__hash__value mt-5 d-flex ai-center font-mono medium lh-100 c-txt-main wb-ba fs-20"]')
            waller=wallet[0].name
            print(wallet)
